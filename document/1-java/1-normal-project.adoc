...
** `AwsCredentials` - AWSの認証情報を指定します(S3をレポジトリーに使っている場合)
*** `accessKey` - AWSのアクセスキーを指定します
*** `secretKey` - AWSのシークレットキーを指定します
...
認証情報をビルドスクリプトに直接書いておくと、危険です。また、複数のビルドスクリプトに直接書いてあると、認証情報の管理も煩わしくなります。

一般的にこのような認証情報はビルドスクリプトとは別のファイルに保管します。

`$GRADLE_USER_HOME/gradle.properties`(もし、 `$GRADLE_USER_HOME` が設定されていない場合は `$HOME/.gradle/gradle.properties`)ファイルはマシン内にある複数のプロジェクトで共有できるプロパティファイルです。

このファイルに認証情報を保存しておくのがよいでしょう。

CAUTION: プロパティのキーと値の一覧を表示する `properties` タスクは `$HOME/.gradle/gradle.properties` の情報も表示するので、絶対に人前では実行しないでください。
